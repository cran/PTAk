...
data Crime;
   ...
   NewHampshire   3.2 10.7  23.2  76.0 1041.7 2343.9 293.4
   NewJersey      5.6 21.0 180.4 185.1 1435.8 2774.5 511.5
   ...
   ;
 c.dat_scan(file="C:/DidierPC/PTA-k/man/crimerate.sas",what=list(character(14),
 double(0),double(0),double(0),double(0),double(0),double(0),double(0)),skip=6,nlines=50)

 crimerate.mat_cbind(c.dat[[2]],c.dat[[3]],c.dat[[4]],c.dat[[5]],c.dat[[6]],c.dat[[7]],c.dat[[8]])
 dimnames(crimerate.mat)_list(c.dat[[1]],c("Murder", "Rape", "Robbery", "Assault",
            "Burglary","Larceny", "AutoTheft"))

crimerate.mat_sweep(crimerate.mat,2,apply(crimerate.mat,2,mean))
crimerate.mat_sweep(crimerate.mat,2,sqrt(apply(crimerate.mat,2,var)),FUN="/")
cri.svd_SVDgen.R(crimerate.mat)
plot.PTAk.R(cri.svd,scree=TRUE)
par(new=TRUE)
RiskJack.R(cri.svd,qchoix=1:7,mod=NULL,max=NULL,rescaled=TRUE,axes=FALSE,ann=FALSE)
par(new=FALSE)
